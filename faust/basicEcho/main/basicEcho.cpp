...
class mydsp : public dsp {
 ...
 public:
	...
	void metadata(Meta* m) { 
		...
		m->declare("filename", "basicEcho.dsp");
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("basicEcho");
		ui_interface->declare(&fHslider1, "7", "");
		ui_interface->addHorizontalSlider("echoFeedback", &fHslider1, 0.0f, 0.0f, 1.0f, 0.00999999978f);
		ui_interface->declare(&fHslider2, "7", "");
		ui_interface->addHorizontalSlider("echoLPF", &fHslider2, 2500.0f, 1000.0f, 10000.0f, 0.00999999978f);
		ui_interface->declare(&fHslider0, "7", "");
		ui_interface->addHorizontalSlider("echoTime", &fHslider0, 0.100000001f, 0.100000001f, 2.5f, 0.00999999978f);
		ui_interface->closeBox();
	}
	...
	virtual void compute(int count, FAUSTFLOAT** inputs, FAUSTFLOAT** outputs) {
		...
		float fSlow0 = (4.99999987e-05f * float(fHslider0));
		float fSlow1 = (0.00100000005f * float(fHslider1));
		float fSlow2 = (0.00100000005f * float(fHslider2));
		for (int i = 0; (i < count); i = (i + 1)) {
			float fTemp0 = (float(input0[i]) + float(input1[i]));
			fRec6[0] = (fSlow0 + (0.999949992f * fRec6[1]));
			float fTemp1 = (fConst0 * fRec6[0]);
			float fTemp2 = ((fRec2[1] != 0.0f) ? (((fRec3[1] > 0.0f) & (fRec3[1] < 1.0f)) ? fRec2[1] : 0.0f) : (((fRec3[1] == 0.0f) & (fTemp1 != fRec4[1])) ? 0.0009765625f : (((fRec3[1] == 1.0f) & (fTemp1 != fRec5[1])) ? -0.0009765625f : 0.0f)));
			fRec2[0] = fTemp2;
...
basicEcho::basicEcho(int sample_rate, int buffer_size)
{
    fUI = new MapUI();
...
void basicEcho::setParamValue(const std::string& path, float value)
{
...
extern "C" void app_main()
{
    ...
    basicEcho* DSP = new basicEcho(48000, 32);
    DSP->start();
