...
//			ESP_LOGI("Queue rx", "pin %d", ev.pin);
			if ((ev.pin == 13) && (ev.event == BUTTON_DOWN)) {
				...
				{
					echoFeedback = echoFeedback * ECHOFBWIDTHFACTOR;
				}
				else 
				{
				...
				echoFlanger.setParamValue("echoFeedback", echoFeedback);		
			...
			if ((ev.pin == 18) && (ev.event == BUTTON_DOWN)) {
				...
				echoFlanger.setParamValue("Width", width);	
			...
			if ((ev.pin == 19) && (ev.event == BUTTON_DOWN)) {
				...
				echoFlanger.setParamValue("Rate", rate);	
